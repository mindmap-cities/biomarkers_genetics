...
opal.assign.table.tibble(erasmus_opal, 'CoP4_Baseline','CLSA_1.CoP4_Baseline', variables =list("BLD_HSCRP_COM","BLD_TSH_COM","BLD_FT4_COM","BLD_ALB_COM","BLD_ALT_COM","BLD_VITD_COM","BLD_TRIG_COM"))

CLSA_cop0 <- opal.execute(erasmus_opal,'CoP4_Baseline')

#opal.assign.table.tibble(erasmus_opal, 'CoP_FU1','CLSA_1.CoP_FU1',  variables=list())

#CLSA_cop1 <- opal.execute(erasmus_opal,'CoP_FU1')

```
## Assign participant id as first column of your harmonized dataframes
```{r, echo=FALSE}
bio_CLSA_COP_0 <- data.frame(id = CLSA_cop0$id)
#bio_CLSA_COP_1 <- data.frame(id = CLSA_cop1$id)
```
...
**Harmonization comment**: Available in 6 studies: CLSA, HUNT, LASA, MESA, RS, HAPIEE (baseline only). In this study, values "less than 0.1 - exact value undetermined" are coded as null.    
**R script**:  
```{r, echo=TRUE}
bio_CLSA_COP_0$bio_crp_0 <- ifelse(CLSA_cop0$BLD_HSCRP_COM =='-2222' | CLSA_cop0$BLD_HSCRP_COM =='-8888',NA,CLSA_cop0$BLD_HSCRP_COM)
```
### **Variable label**: C-Reactive Protein  
...
**Harmonization status**:  complete  
**Harmonization comment**: Available in 4 studies: CLSA, HUNT, LASA, RS.  In this study, values "less than 0.1 - exact value undetermined" are coded as null.    
**R script**:  
```{r, echo=TRUE}
bio_CLSA_COP_0$bio_tsh_0 <- ifelse(CLSA_cop0$BLD_TSH_COM =='-2222' | CLSA_cop0$BLD_TSH_COM =='-8888',NA,CLSA_cop0$BLD_TSH_COM)
```
### **Variable label**: Thyroid Stimulating Hormone  
...
**Harmonization status**:  complete  
**Harmonization comment**: Available in 4 studies: CLSA, HUNT, LASA, RS. In this study, values that are "greater than 100 - exact value undetermined" or "less than 0.3 - exact value undetermined" are coded as null.  
**R script**:  
```{r, echo=TRUE}
bio_CLSA_COP_0$bio_t4_0 <- ifelse(CLSA_cop0$BLD_FT4_COM =='-2222' | CLSA_cop0$BLD_FT4_COM =='-1111',NA,CLSA_cop0$BLD_FT4_COM)
```
### **Variable label**: Thyroxine  
...
**R script**:  
```{r, echo=TRUE}
bio_CLSA_COP_0$bio_alb_0 <- ifelse(CLSA_cop0$BLD_ALB_COM =='-8888',NA,CLSA_cop0$BLD_ALB_COM)
```
### **Variable label**: Albumin  
...
**Harmonization status**:  complete  
**Harmonization comment**: Available in 5 studies: CLSA, LASA, RECORD, MESA, RS. In this study, values "less than 5 - exact value undetermined" are coded as null.   
**R script**:  
```{r, echo=TRUE}
bio_CLSA_COP_0$bio_alt_0 <- ifelse(CLSA_cop0$BLD_ALT_COM =='-8888' | CLSA_cop0$BLD_ALT_COM =='-2222',NA,CLSA_cop0$BLD_ALT_COM)
```
### **Variable label**: Alanine Aminotransferase  
...
**Category coding**: 
**Harmonization status**:  complete  
**Harmonization comment**: Available in 5 studies: CLSA, HUNT, LASA, MESA, RS. In this study, values "greater than 350 - exact value undetermined" and values "less than 8 - exact value undetermined" are coded as null.  
**R script**:  
```{r, echo=TRUE}
bio_CLSA_COP_0$bio_vitd_0 <- ifelse(CLSA_cop0$BLD_VITD_COM =='-8888' | CLSA_cop0$BLD_VITD_COM =='-2222' | CLSA_cop0$BLD_VITD_COM =='-1111',NA,CLSA_cop0$BLD_VITD_COM)
```
### **Variable label**: Vitamin D 
...
**R script**:  
```{r, echo=TRUE}
bio_CLSA_COP_0$bio_tg_0 <- ifelse(CLSA_cop0$BLD_TRIG_COM =='-8888',NA,CLSA_cop0$BLD_TRIG_COM)
```
### **Variable label**: Triglycerides 
